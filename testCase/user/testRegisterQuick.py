...
localRegisterQuick_xls = commonclass.get_xls("userCase.xlsx", "registerquick")
...
class RegisterQuick(unittest.TestCase):
    ...
    def testRegisterQuick(self):
        ...
        """
        # set url
        self.url = commonclass.get_url_from_xml('registerQuick')
    ...
    def checkResult(self):
        ...
        commonclass.show_return_msg(self.response)

        if self.result == '0':
            email = commonclass.get_value_from_return_json(self.info, 'member', 'email')
        ...
        if self.result == '1':
            ...
            if self.case_name == 'registerQuick_EmailExist':
                sql = commonclass.get_sql('newsitetest', 'rs_member', 'delete_user')
